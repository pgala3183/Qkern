"""
Compare FP16 GEMV variants with identical CUDA-event methodology.

Reports median latency, speedup vs naive, logical bytes, logical bandwidth.
Does not claim superiority beyond the measured numbers.
"""
...
def logical_bytes(n: int, k: int, variant: str) -> dict[str, float]:
    ...
    y = float(n * 4)
    if variant == "naive":
        x = float(n * k * 2)  # each of N threads reads full x
    elif variant == "x_smem":
        x = float(k * 2)  # x staged once from global (accounting model)
    else:
...
def main() -> int:
    ...
    parser.add_argument(
        "--shapes",
        default="64x256,256x4096,1024x4096,4096x4096",
    )
    ...
    meta = {
        ...
        "experiment": "fp16_gemv_x_smem",
        "diagnostics": collect_diagnostics(),
        "logical_byte_model": {
            "naive": "N*K*2 + N*K*2 + N*4",
            "x_smem": "N*K*2 + K*2 + N*4",
        },
    }
    ...
    json_path = args.out_dir / f"fp16_gemv_x_smem_{stamp}.json"
    csv_path = args.out_dir / f"fp16_gemv_x_smem_{stamp}.csv"
